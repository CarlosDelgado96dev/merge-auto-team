...
MSG="${1:-automatic merge from hot-fix}"
...
remote_commits=$(git log origin/hot-fix --first-parent --since="1 week ago" --no-merges --pretty=format:"%H" --grep='Version [0-9]\+\.[0-9]\+\.[0-9]\+ - [0-9]\{2\}/[0-9]\{2\}' --grep="readme" --grep="Merge branch" --invert-grep)
